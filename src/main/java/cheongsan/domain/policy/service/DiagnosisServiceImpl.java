...
public class DiagnosisServiceImpl implements DiagnosisService {
    ...
    public DiagnosisDTO processDiagnosis(UserDiagnosisDTO request) {
        ...
        Long diagnosisId = new Long(determineUserStatus(answers));

        // (3) 판단된 상태/조건을 토대로 정책 조회(실제 추천 로직은 정책 조건 매칭 규칙에 따라 달라짐)
        DiagnosisEntity policy = policyMapper.getDiagnosisResult(diagnosisId);
    ...
    private int determineUserStatus(List<UserDiagnosisDTO.Answer> answers) {
        ...
        // 연체 상황을 우선적으로 판단
        if (q1 == null) {
            return 0; // 답변 불완전 시 기본값
        }

        switch (q1) {
            case 1: // 연체 없음
                return 0; // 예방적 상담 또는 일반 금융상품 추천
