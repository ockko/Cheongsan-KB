...
public class Calculator {
    ...
    public CalculateDTO calculateEqualPayment(BigDecimal interestRate, BigDecimal months, BigDecimal loanAmount) {
        if (interestRate.equals(BigDecimal.ZERO)) {
            return new CalculateDTO(loanAmount.negate(), loanAmount.negate().divide(months, MATH_CONTEXT));
        }
        if (months.signum() < 0) {
        ...
        return new CalculateDTO(monthlyPayment.multiply(months), monthlyPayment);
    }
}
