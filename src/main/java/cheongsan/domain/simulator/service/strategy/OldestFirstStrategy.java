...
public class OldestFirstStrategy implements RepaymentStrategy {
    ...
    public RepaymentResponseDTO simulate(RepaymentRequestDTO request, List<LoanDTO> originalLoans) {
        ...
        List<SimulatedLoanDTO> activeLoans = loans.stream()
                .map(loan -> {
                    LocalDate startDate = calculateStartDate(loan.getPaymentDate(), LocalDate.now());
    ...
    private LocalDate calculateStartDate(LocalDate nextPaymentDate, LocalDate today) {
        if (nextPaymentDate == null) return today;
        LocalDate scheduledThisMonth = nextPaymentDate.withMonth(today.getMonthValue()).withYear(today.getYear());
        return scheduledThisMonth.isBefore(today) ? scheduledThisMonth.plusMonths(1) : scheduledThisMonth;
    }
