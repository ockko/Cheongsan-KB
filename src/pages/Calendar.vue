...
const handleDateClick = async (date) => {
    ...
    if (hasTransactionData || hasLoanData) {
        if (selectedDate.value === date) {
            ...
            // 상세 데이터 로드
            const dateString = calendarStore.formatDateString(currentYear.value, currentMonth.value, date);
...
<template>
    <div :class="styles.pageWrapper">
        <div :class="styles.page">
            ...
            <!-- 로딩 표시 -->
            <div v-if="calendarStore.loading" :class="styles.loading">데이터를 불러오는 중...</div>

            <CalendarHeader :year="currentYear" :month="currentMonth - 1" @change-month="changeMonth" />
